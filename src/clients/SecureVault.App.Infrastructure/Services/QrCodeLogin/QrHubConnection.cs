...
public class QrHubConnection : IQrHubConnection
{
    ...
    public async Task<string> StartConnectionAsync(string? channelId, CancellationToken cancellationToken = default)
    {
        ...
        if (string.IsNullOrEmpty(channelId))
        {
            ...
            _logger.LogInformation("No channel ID provided. Generated new ChannelId: {ChannelId}", channelId);
        }
        else
        {
            _logger.LogInformation("Joining existing ChannelId: {ChannelId}", channelId);
        }
        ...
        async ValueTask<WebSocket> CreateWebSocketFactoryAsync(WebSocketConnectionContext context, CancellationToken factoryCancellationToken)
        {
            ...
            try
            {
                ...
                _logger.LogDebug("WebSocketFactory: Connecting to {Uri} with DPoP and Channel headers...", context.Uri);
                await dws.ConnectAsync(context.Uri, factoryCancellationToken);
                return dws;
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "WebSocketFactory failed during DPoP proof creation or connection.");
                dws.Dispose();
                throw;
            }
        }
        ...
        _hubConnection.On<string, string>("ReceiveMessage", async (type, payload) =>
        {
            ...
            {
                _logger.LogError(ex, "Error in OnMessageReceived subscriber for type {MessageType}.", type);
            }
        });
        ...
        _hubConnection.On<string>("Error", async (errorMessage) =>
        {
            ...
            {
                _logger.LogError(ex, "Error in OnErrorReceived subscriber.");
            }
        });
        ...
        _logger.LogInformation("QR Hub connection started. ConnectionId: {ConnectionId}", _hubConnection.ConnectionId);
        return _channelId;
    }
    public async Task<string> SwitchChannelAsync(CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            await _hubConnection.InvokeAsync("SwitchToNewChannel", newChannelId, cancellationToken);
    ...
    private Task OnReconnected(string? newConnectionId)
    {
        ...
        _logger.LogInformation("Channel {ChannelId} was re-joined automatically via OnConnectedAsync logic (server-side).", _channelId);
        return Task.CompletedTask;
    }
