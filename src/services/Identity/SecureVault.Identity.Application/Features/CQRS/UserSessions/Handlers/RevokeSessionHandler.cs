    ...
    public class RevokeSessionHandler : IRequestHandler<RevokeSessionCommand, Result>
    {
        ...
        public async Task<Result> Handle(RevokeSessionCommand request, CancellationToken cancellationToken)
        {
            try
            {
                ...
                {
                    _logger.LogWarning("Zaten var olmayan veya iptal edilmiş bir oturum sonlandırılmaya çalışıldı. SessionId: {SessionId}", request.SessionId);
                    return Result.Success();
                }

                if (sessionToRevoke.UserId != request.CurrentUserId)
                {
                    _logger.LogCritical("GÜVENLİK UYARISI: Yetkisiz oturum sonlandırma denemesi! Yapan Kullanıcı: {ActorId}, Hedef Oturum Sahibi: {TargetId}",
                        request.CurrentUserId, sessionToRevoke.UserId);
                ...
                _logger.LogInformation("Oturum başarıyla sonlandırıldı. SessionId: {SessionId}, UserId: {UserId}", request.SessionId, request.CurrentUserId);

                return Result.Success();
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Oturum sonlandırma sırasında beklenmedik bir hata oluştu. SessionId: {SessionId}", request.SessionId);
