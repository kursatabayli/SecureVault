...
public class RequestLoginChallengeHandler : IRequestHandler<RequestLoginChallengeCommand, Result<ChallengeDto>>
{
    ...
    public async Task<Result<ChallengeDto>> Handle(RequestLoginChallengeCommand request, CancellationToken cancellationToken)
    {
        try
        {
            ...
            if (user is null)
            {
                _logger.LogWarning("Login challenge request failed: User not found. Email: {Email}", request.Email);
            ...
            _logger.LogInformation("Login challenge created and cached successfully for User: {UserId}. Email: {Email}", user.Id, request.Email);

            return loginChallengeDto;
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "An unexpected error occurred during the login challenge request. Email: {Email}", request.Email);
