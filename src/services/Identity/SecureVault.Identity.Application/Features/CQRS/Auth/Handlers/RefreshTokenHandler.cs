...
public class RefreshTokenHandler : IRequestHandler<RefreshTokenCommand, Result<AuthResponse>>
{
    ...
    public async Task<Result<AuthResponse>> Handle(RefreshTokenCommand request, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            _logger.LogInformation("Token refreshed successfully for User: {UserId}, Session: {SessionId}", session.UserId, session.Id);
        ...
        {
            _logger.LogError(ex, "An unexpected error occurred during the refresh token process. UserId: {UserId}", session.UserId);
