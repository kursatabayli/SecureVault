...
public class EcdsaVerificationService : IEcdsaVerificationService
{
    ...
    private readonly ILogger<EcdsaVerificationService> _logger;

    public EcdsaVerificationService(ILogger<EcdsaVerificationService> logger)
    {
        _logger = logger;
    ...
    public bool VerifySignature(string message, string signatureHex, byte[] publicKey)
    {
        try
        {
            ...
            bool isValid = verifier.VerifySignature(msgHash, r, s);
            if (!isValid)
                _logger.LogWarning("ECDSA signature verification failed (invalid signature). Message: {Message}", message);

            return isValid;
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "An unexpected error occurred during ECDSA signature verification. Message: {Message}", message);
            return false;
        }
    }
}
