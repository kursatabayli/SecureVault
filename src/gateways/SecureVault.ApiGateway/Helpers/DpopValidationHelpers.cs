...
public static class DpopValidationHelpers
{
    ...
    public static async Task<(bool IsValid, string? Jkt)> ValidateDpopProof(string dpopProofJwt, string httpMethod, string httpUrl, string? accessToken, IDpopJtiCache jtiCache)
    {
        ...
        try
        {
            ...
            var iatTime = validatedProof.Payload.IssuedAt;

            if (iatTime == DateTime.MinValue || iatTime < DateTime.UtcNow.AddMinutes(-5) || iatTime > DateTime.UtcNow.AddMinutes(5))
            {
                Log.Warning("[DPoP] Error: 'iat' (timestamp) is missing or outside the valid time range.");
                return (false, null);
            }
